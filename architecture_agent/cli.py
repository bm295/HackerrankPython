...
from architecture_agent.analyzer import build_snapshot
...
def run_analysis(args: argparse.Namespace) -> AnalysisReport:
    config = load_config()
    ...
    topic = select_topic(
        TOPICS,
        seed,
        applicable=lambda item: (
            item.source_urls
            and (
                normalized_topic is None
                or item.name.lower() == normalized_topic
                or item.id.lower() == normalized_topic
            )
        ),
    )
    ...
    return AnalysisReport(
        ...
        architecture={
            "style": snapshot.type,
            "buildingBlocks": ["CLI", "Scanner", "Analyzer", "Topic Selector", "Research Provider"],
